...
def list_members(keyword: str | None = None, is_active: bool | None = None) -> list[dict[str, Any]]:
    ...
    if keyword:
        clauses.append("(name like ? or department like ? or role like ? or phone like ?)")
        pattern = f"%{keyword}%"
...
def create_member(payload: MemberCreate) -> dict[str, Any]:
    ...
    with get_connection() as connection:
        cursor = connection.execute(
            """
            insert into members (name, department, role, phone, memo, is_active)
            values (?, ?, ?, ?, ?, 1)
            """,
            (payload.name, payload.department, payload.role, payload.phone, payload.memo),
        )
        connection.commit()
